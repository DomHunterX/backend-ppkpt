...
async function run() {
  try {
  const identity_number = 'MHS0001';
  const rawPassword = 'test1234';
  const role = 'mahasiswa';
  const full_name = 'Mahasiswa Test';
  ...
  const phone_number = '081234567890';
    ...
    if (existsRows.length) {
      ...
      // Insert into users (schema: id, identity_number, password, role, created_at)
    ...
    // Insert mahasiswa profile linked to users.id (upsert-like: skip if nim exists)
